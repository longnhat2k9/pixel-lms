...
export default function TeacherCourses() {
  ...
  return (
    <Layout user={user}>
      ...
      <p className="text-mute mb-6 text-sm">Các khóa học admin đã cấp quyền cho bạn. Bạn có thể tạo chương, bài, nội dung.</p>
      {error && <div className="mb-4 text-sm text-danger">{error}</div>}
      <div className="grid md:grid-cols-3 gap-4">
        {courses.map((c) => (
          <Link key={c.id} href={`/teacher/courses/${c.id}`} className="pxl-card p-5 block hover:border-accent transition">
