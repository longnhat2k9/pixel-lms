  ...
  return (
    <Layout user={user}>
      ...
      <div className="space-y-3">
        {questions.map((q, idx) => (
          <div key={q.id} className="pxl-card p-4 flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="text-xs text-mute mb-1">{TYPE_LABEL[q.type]} · {q.points} điểm</div>
              <div className="text-sm">
                <span className="text-mute">{idx + 1}.</span> <MarkdownRenderer content={q.content} className="inline" />
              </div>
              {(q.type === "choice2" || q.type === "choice4") && (
                <ul className="text-xs text-mute mt-1 list-disc list-inside">
                  {(q.data?.options || []).map((o, i) => (
                    <li key={i} className={String(i) === String(q.correct_answer?.value) ? "text-accent2" : ""}>
                      <MarkdownRenderer content={o} className="inline" />
                    </li>
                  ))}
                </ul>
              )}
              {q.type === "fill_blank" && (
                <div className="text-xs text-accent2 mt-1">Đáp án: {q.correct_answer?.value}</div>
              )}
            </div>
